app.controller('PostCtrl', ['$scope', function($scope) {
  ...
  $scope.editMode = function (index, post) {
  ...
  $scope.createPost = function() {
    ...
    $scope.postsDB.push({ title: $scope.title_input, date: $scope.date_input, content: $scope.content_input});
    $scope.title_input = $scope.date_input = $scope.content_input = "";
  };

  $scope.savePost = function() {
    console.log("save click")
    $scope.postsDB[$scope.postIndex] = $scope.post;

  };

  $scope.deletePost = function($index) {
    console.log("delete click")
    $scope.postsDB.splice($index, 1);
  };
